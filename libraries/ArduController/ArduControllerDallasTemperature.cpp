...
ArduControllerDallasTemperature::ArduControllerDallasTemperature(ArduControllerOneWire* _oneWire)
  #if REQUIRESALARMS
...
// initialize the bus
void ArduControllerDallasTemperature::begin(void)
{
...
uint8_t ArduControllerDallasTemperature::getDeviceCount(void)
{
  return devices;
}

// returns true if address is valid
bool ArduControllerDallasTemperature::validAddress(uint8_t* deviceAddress)
{
...
bool ArduControllerDallasTemperature::getAddress(uint8_t* deviceAddress, uint8_t index)
{
  uint8_t depth = 0;
...
bool ArduControllerDallasTemperature::isConnected(uint8_t* deviceAddress)
{
  ScratchPad scratchPad;
...
bool ArduControllerDallasTemperature::isConnected(uint8_t* deviceAddress, uint8_t* scratchPad)
{
  readScratchPad(deviceAddress, scratchPad);
...
void ArduControllerDallasTemperature::readScratchPad(uint8_t* deviceAddress, uint8_t* scratchPad)
{
  // send the command
  _wire->reset();
...
void ArduControllerDallasTemperature::writeScratchPad(uint8_t* deviceAddress, const uint8_t* scratchPad)
{
  _wire->reset();
  _wire->select(deviceAddress);
...
bool ArduControllerDallasTemperature::readPowerSupply(uint8_t* deviceAddress)
{
  bool ret = false;
  _wire->reset();
  _wire->select(deviceAddress);
...
void ArduControllerDallasTemperature::setResolution(uint8_t newResolution)
{
...
bool ArduControllerDallasTemperature::setResolution(uint8_t* deviceAddress, uint8_t newResolution)
{
  ScratchPad scratchPad;
...
uint8_t ArduControllerDallasTemperature::getResolution()
{
...
uint8_t ArduControllerDallasTemperature::getResolution(uint8_t* deviceAddress)
{
...
void ArduControllerDallasTemperature::setWaitForConversion(bool flag)
{
	waitForConversion = flag;
}

// gets the value of the waitForConversion flag
bool ArduControllerDallasTemperature::getWaitForConversion()
{
...
void ArduControllerDallasTemperature::requestTemperatures()
{
  _wire->reset();
  _wire->skip();
...
bool ArduControllerDallasTemperature::requestTemperaturesByAddress(uint8_t* deviceAddress)
{
  // check device
  ScratchPad scratchPad;
...
bool ArduControllerDallasTemperature::requestTemperaturesByIndex(uint8_t deviceIndex)
{
  DeviceAddress deviceAddress;
...
float ArduControllerDallasTemperature::getTempCByIndex(uint8_t deviceIndex)
{
  DeviceAddress deviceAddress;
  getAddress(deviceAddress, deviceIndex);
...
// Fetch temperature for device index
float ArduControllerDallasTemperature::getTempFByIndex(uint8_t deviceIndex)
{
  return toFahrenheit(getTempCByIndex(deviceIndex));
}

// reads scratchpad and returns the temperature in degrees C
float ArduControllerDallasTemperature::calculateTemperature(uint8_t* deviceAddress, uint8_t* scratchPad)
{
...
float ArduControllerDallasTemperature::getTempC(uint8_t* deviceAddress)
{
...
float ArduControllerDallasTemperature::getTempF(uint8_t* deviceAddress)
{
  return toFahrenheit(getTempC(deviceAddress));
}

// returns true if the bus requires parasite power
bool ArduControllerDallasTemperature::isParasitePowerMode(void)
{
  return parasite;
}
...
void ArduControllerDallasTemperature::setHighAlarmTemp(uint8_t* deviceAddress, char celsius)
{
...
// accepts a float, but the alarm resolution will ignore anything
// after a decimal point.  valid range is -55C - 125C
void ArduControllerDallasTemperature::setLowAlarmTemp(uint8_t* deviceAddress, char celsius)
{
  // make sure the alarm temperature is within the device's range
  if (celsius > 125) celsius = 125;
  else if (celsius < -55) celsius = -55;
...
char ArduControllerDallasTemperature::getHighAlarmTemp(uint8_t* deviceAddress)
{
  ScratchPad scratchPad;
...
// DEVICE_DISCONNECTED for an address
char ArduControllerDallasTemperature::getLowAlarmTemp(uint8_t* deviceAddress)
{
  ScratchPad scratchPad;
...
void ArduControllerDallasTemperature::resetAlarmSearch()
{
...
bool ArduControllerDallasTemperature::alarmSearch(uint8_t* newAddr)
{
  uint8_t i;
  char lastJunction = -1;
  uint8_t done = 1;
...
bool ArduControllerDallasTemperature::hasAlarm(uint8_t* deviceAddress)
{
  ScratchPad scratchPad;
  if (isConnected(deviceAddress, scratchPad))
  {
...
bool ArduControllerDallasTemperature::hasAlarm(void)
{
  DeviceAddress deviceAddress;
  resetAlarmSearch();
...
void ArduControllerDallasTemperature::processAlarms(void)
{
  resetAlarmSearch();
...
void ArduControllerDallasTemperature::setAlarmHandler(AlarmHandler *handler)
{
  _AlarmHandler = handler;
}

// The default alarm handler
void ArduControllerDallasTemperature::defaultAlarmHandler(uint8_t* deviceAddress)
{
}
...
float ArduControllerDallasTemperature::toFahrenheit(float celsius)
{
  return (celsius * 1.8) + 32;
}

// Convert float fahrenheit to celsius
float ArduControllerDallasTemperature::toCelsius(float fahrenheit)
{
...
void* ArduControllerDallasTemperature::operator new(unsigned int size) // Implicit NSS obj size
{
  void * p; // void pointer
...
void ArduControllerDallasTemperature::operator delete(void* p)
{
